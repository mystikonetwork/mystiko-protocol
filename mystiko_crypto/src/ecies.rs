...
pub fn generate_secret_key() -> Result<[u8; 32], CryptoError> {
...
pub fn unpack_public_key(public_key: &[u8]) -> Result<([u8; 32], [u8; 32]), CryptoError> {
    let point = babyjubjub_unpack_point(public_key)?;
    Ok((fr_to_bytes(&point.x)?, fr_to_bytes(&point.y)?))
}

pub fn public_key_from_unpack_point(x: &[u8], y: &[u8]) -> Result<[u8; 32], CryptoError> {
    babyjubjub_public_key(x, y)
}

pub fn encrypt(plain: &BigUint, pk: &[u8], common_sk: &[u8]) -> Result<BigUint, CryptoError> {
    ...
    Ok(mod_floor(&(plain.clone() + hm), &FIELD_SIZE))
}

pub fn decrypt(encrypted: &BigUint, sk: &[u8], common_pk: &[u8]) -> Result<BigUint, CryptoError> {
    ...
    match encrypted.cmp(&hm) {
        std::cmp::Ordering::Less => {
            ...
            Ok(FIELD_SIZE.clone() - dec)
        }
        std::cmp::Ordering::Equal => Ok(BigUint::from(0u32)),
        std::cmp::Ordering::Greater => Ok(mod_floor(&(encrypted - hm), &FIELD_SIZE)),
    }
}
