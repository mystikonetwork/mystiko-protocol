...
async fn test_calc_zeros() {
    ...
    let zeros = calc_zeros(fist_zero, &32u32).unwrap();
    assert_eq!(
        zeros[31],
        BigUint::parse_bytes(
...
async fn test_new_merkle_tree() {
    ...
    assert_eq!(tree3.root(), hash_two(&zero_element, &zero_element).unwrap());

    let tree4 = MerkleTree::new(Some(elements), Some(0), Some(zero_element));
    assert!(matches!(tree4.err().unwrap(), MerkleTreeError::MerkleTreeIsFull));
}
...
async fn test_insert() {
    ...
    assert!(matches!(result.err().unwrap(), MerkleTreeError::MerkleTreeIsFull));
...
async fn test_path() {
    ...
    assert_eq!(result1.0, vec![e2.clone(), hash_two(&e3, &default_zero).unwrap()]);

    let result2 = tree.path(1).unwrap();
    assert_eq!(result2.1, vec![1, 0]);
    assert_eq!(result2.0, vec![e1.clone(), hash_two(&e3, &default_zero).unwrap()]);

    let result3 = tree.path(2).unwrap();
    assert_eq!(result3.1, vec![0, 1]);
    assert_eq!(result3.0, vec![default_zero, hash_two(&e1, &e2).unwrap()]);
